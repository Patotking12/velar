...
;; Could also store balance and compare to previous but that adds a lot of code.
(define-public (send-revenue (pool uint) (is-token0 bool) (amt uint))
    (let 
        (
            (epoch (contract-call? .staking-core current-epoch))
...
(define-private (do-claim (user principal) (pool uint) (epoch uint))
    (map-set claims
        {
...
(define-read-only (has-claimed-epoch (user principal) (pool uint) (epoch uint))
  (is-some (map-get? claims {user: user, pool: pool, epoch: epoch}))
)

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; low level API
(define-private (do-distribute (user principal) (token0 <ft-trait>) (token1 <ft-trait>) (amts {token0: uint, token1: uint}))
...
(define-read-only (calc-distribute (share {staked: uint, total: uint}) (rev {token0: uint, token1: uint}))
    {
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; distribute-epoch
(define-public (distribute-epoch (user principal) (id uint) (token0 <ft-trait>) (token1 <ft-trait>) (epoch uint))
    (let 
        (
            (pool  (contract-call? .core do-get-pool id))
            (share (try! (contract-call? .staking-core get-share-at user epoch)))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; distribute-epochs
(define-private (distribute-epochs-step (epoch uint) (args {user: principal, pool: uint, token0: <ft-trait>, token1: <ft-trait>}))
    (let 
        (
            (event_ (unwrap-panic (distribute-epoch (get user args) (get pool args) (get token0 args) (get token1 args) epoch)))
        )
        args
    )
)

(define-public (distribute-epochs (user principal) (pool uint) (token0 <ft-trait>) (token1 <ft-trait>) (epochs (list 10 uint))) ;;XXX: MAX-EPOCHS
    (let 
        (
            (args 
                {
...
(define-read-only
  ...
  (let ((share   (try! (contract-call? .staking-core get-share-at user epoch)))
...
(define-read-only
  ...
  (let ((reward (unwrap-panic (get-reward (get user (get args state))
...
(define-read-only (mkepochs (start-epoch uint))
  (let ((current (contract-call? .staking-core current-epoch))
